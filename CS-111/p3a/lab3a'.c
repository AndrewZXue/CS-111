...
void indirect_block_reference(int fd, int indierct_start, int level, int inode_index, int block_index) {
    ...
    if(block_index != 0) {
        pread(fd, &indirect_block, 1024, block_index*1024);
        int i;
        for (i=0; i<256; i++) {
            int curr_offset = indierct_start + i;
            if(indirect_block[i] != 0) 
                dprintf(logfd, "INDIRECT,%d,%d,%d,%d,%d\n",inode_index,level,curr_offset,block_index,indirect_block[i]);
            if(level > 1)
                indirect_block_reference(fd, curr_offset, level - 1, inode_index, indirect_block[i]);
        }
    }
}
...
int main(int argc, char* argv[])
{
	...
	pread(fd, &superblock, superblock_bufsize, 1024);
	...
	{
		pread(fd, &group[i], group_bufsize, block_start_offset + i * group_bufsize);
 	...
 	for(j = 0; j != group_count; j++)
 	{
 		...
 		for(k = 0; k != bsize; k++)
 		{
 			...
 			pread(fd, &buffer, 1, group[i].bg_block_bitmap * bsize + k);
 			int n;
 			for(n = 0; n != 8; n++)
 			{
 				if ((buffer & (mask << n)) == 0)
 	...
 	for(j = 0; j != group_count; j++)
 	{
 		...
 		{
 			int buff_start = group_start + 8 * k;
 			pread(fd, &buffer, 1, group[i].bg_inode_bitmap * bsize + k);
 			int n;
 			for(n = 0; n != 8; n++)
 			{
 				if ((buffer & (mask << n)) == 0)
 	...
 	for (j = 0; j != group_count; j++)
 	{
 		...
 		for(k = 1; k <= (int)superblock.s_inodes_per_group; k++)
 		{
 			...
 			pread(fd, &inode, inode_bufsize, it_offset);
 			if(inode.i_mode && inode.i_links_count)
 			{
 				...
 				if(filetype == 'd')
 				{
 					...
 					for (p = 0; p != 12; p++)
 					{
 						...
 						{
 							pread(fd, &directory, dir_bufsize, DIRECTORY_START + directory_offset);
